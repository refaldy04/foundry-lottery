...
contract Raffle is VRFConsumerBaseV2Plus {
    ...
    uint32 private immutable i_callbackGasLimit; // 100k gas limit for the callback
     ...
     * The following should be true in order for upkeep needed to return true:
     * 1. The time interval has passed
     * 2. The lottery is in the OPEN state
     * 3. The contract has ETH (i.e., players have entered)
    ...
    function performUpkeep(bytes calldata /* performData */) external {
        ...
        s_raffleState = RaffleState.CALCULATING; // Set state to CALCULATING to prevent re-entrancy attacks
    ...
    // ini callback function yang akan dipanggil oleh Chainlink VRF
    function fulfillRandomWords(
        ...
        s_raffleState = RaffleState.OPEN; // Set state back to OPEN for the next round
